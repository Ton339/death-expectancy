...
const steps: StepConfig[] = [
  { key: "age", title: "เวลาที่ใช้ชีวิตมาแล้ว", unit: "ปี", min: 0, max: 69, default: 20 },
  { key: "weight", title: "น้ำหนัก", unit: "กิโลกรัม", min: 30, max: 150, default: 60 },
  { key: "height", title: "ส่วนสูง", unit: "เซนติเมตร", min: 100, max: 220, default: 160 },
  { key: "sleep", title: "ชั่วโมงในการหลับใหล", unit: "ชั่วโมงต่อวัน", min: 0, max: 12, default: 8 },
  { key: "diet", title: "มื้ออาหารที่ครบ 5 หมู่", unit: "วันต่อสัปดาห์", min: 0, max: 7, default: 3 },
  { key: "alcohol", title: "ดื่มแอลกอฮอล์", unit: "ครั้งต่อเดือน", min: 0, max: 30, default: 2 },
  { key: "smoke", title: "สูบบุหรี่", unit: "มวนต่อวัน", min: 0, max: 21, default: 0 },
  { key: "exercise", title: "ออกกำลังกาย", unit: "ครั้งต่อสัปดาห์", min: 0, max: 7, default: 1 },
]
...
function getResultContent(days: number) {
  if (days < 3000) {
    ...
    return {
      ...
      description: "แสงสว่างยังมีอยู่\nแต่ชะตากรรมยังแกว่งไกว\nการกระทำหลังจากนี้จะเป็นผู้กำหนดทิศทาง",
      gradient: "radial-gradient(circle at center, rgba(250, 250, 249, 0.2) 0%, transparent 70%)",
    }
  } else {
    return {
      bg: "bg-zinc-50",
...
export default function DeathExpectancyCalculator() {
  ...
  const resultContent = showResult && resultRef.current !== null 
    ? getResultContent(resultRef.current) 
    : null

  return (
    <div className={`relative min-h-screen w-full overflow-hidden ${showResult && resultContent ? resultContent.bg : 'bg-black'}`}
         style={resultContent?.gradient ? { background: `${resultContent.gradient}, black` } : {}}>
      {/* URL indicator */}
      <div className="absolute top-4 left-4 text-zinc-500 text-sm z-10">/web-url</div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6">
        <AnimatePresence mode="wait">
          {currentStep === -1 && !showResult && (
            ...
            >
              <h1 className="text-2xl text-zinc-100 mb-2">นาฬิกาชีวิตที่ไม่อาจย้อนกลับ</h1>
              <p className="text-zinc-400 mb-8">ความตายยืนรออยู่ปลายทาง...</p>
              <button
                onClick={handleStart}
                className="px-8 py-3 text-zinc-100 border border-zinc-700 hover:bg-zinc-900 transition-colors"
              >
                ยอมรับชะตากรรม
              </button>
            </motion.div>
          ...
          {currentStep >= 0 && !showResult && (
            ...
            >
              <h2 className="text-xl text-zinc-100 mb-8">{steps[currentStep].title}</h2>
              
              <WheelPicker
                min={steps[currentStep].min}
                max={steps[currentStep].max}
                defaultValue={formData[steps[currentStep].key as keyof FormData]}
          ...
          {showResult && resultContent && (
            ...
            >
              <h2 className="text-lg whitespace-pre-line mb-6">{resultContent.title}</h2>
        ...
        {/* Next Button */}
        <AnimatePresence>
          {currentStep >= 0 && !showResult && hasInteracted && (
            <motion.button
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
              ...
              onClick={handleNext}
              className="absolute bottom-12 px-8 py-3 text-zinc-100 border border-zinc-700 hover:bg-zinc-900 transition-colors"
            >
              {currentStep === steps.length - 1 ? "ล่วงรู้วาระสุดท้าย" : "ดำดิ่งต่อไป"}
            </motion.button>
          )}
        </AnimatePresence>
        ...
        {showResult && (
          ...
          >
            เนื้อหานี้มีไว้เพื่อวัตถุประสงค์ในการฝึกการเขียนโปรแกรมเท่านั้น โปรดปรึกษาผู้เชี่ยวชาญหากต้องการคำแนะนำหรือการวินิจฉัยทางการแพทย์
          </motion.p>
        )}
      </div>
    </div>
